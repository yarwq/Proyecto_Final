...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize game directly with 1 player
    numJugadores = 1;
    jugadorActual = 1;
    
    // Initialize the zoo and player's hand
    zoologicos = {};
    manos = {};
    
    // Initialize structures for one player
    zoologicos[1] = {};
    ...
    if (puntuacionEl) {
        puntuacionEl.textContent = 'Score: 0 points';
    }
    
    agregarDropTargets();

    // Event listener for calculate points button
    ...
    if (btnCalcularPuntos) {
        btnCalcularPuntos.addEventListener('click', () => {
            const puntos = calcularPuntos(zoologicos[1]); // Always use player 1
            const puntuacionEl = document.getElementById('puntuacion');
            if (puntuacionEl) {
                puntuacionEl.textContent = `Score: ${puntos} points`;
            }
        });
    }
 ...
 const silenciar = document.getElementById('silenciar-musica');
  ...
  if (iniciarBtn) {
    iniciarBtn.addEventListener('click', function () {
      ...
      if (!cantidad) {
        alert('Select the number of players before starting.');
        return;
      }
  ...
  function generarCamposNombres(numJugadores) {
    ...
    for (let i = 1; i <= numJugadores; i++) {
      ...
      fieldContainer.innerHTML = `
        <div class="nombres-input-label">Player ${i}</div>
        <input type="text" class="nombres-holo-input" placeholder="Name" data-jugador="${i}">
      `;
  ...
  document.addEventListener('click', function(e) {
    if (e.target.closest('.nombres-btn-continue')) {
      ...
      inputs.forEach(input => {
        nombres.push(input.value.trim() || `Player ${input.dataset.jugador}`);
      });
...
function inicializarJuego(jugadores) {
  ...
  for (let j = 1; j <= numJugadores; j++) {
    ...
    manos[j] = [...tiposDeDinos]; // 6 different dinosaurs
  }
...
function colocarDinoEnZona(indice, zona) {
  ...
  // Add a copy of the dinosaur instead of the original
...
// This function is no longer needed as points will only be calculated when clicking the button
function actualizarPuntuacion() {
    // Do nothing, points are calculated only when pressing the button
}
